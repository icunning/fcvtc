...
//    beginInsertRows(tagIdList, rowCount(), rowCount());
...
int CNamesModel::rowCount(const QModelIndex &/*parent*/) const {
    qDebug() << "rowCount" << tagIdList.size();
...
//    qDebug() << "data" << role << row << col << firstNameList[row] << lastNameList[row];
...
//    else
    ...
    namesModel->addName(tagId, firstName, lastName);
//    namesModel->loadNames();

//    int rowCount = namesModel->rowCount();
//    namesModel->beginInsertRows(namesModel->tagIdList, rowCount, rowCount);
//    namesModel->insertRows(namesModel->rowCount(), 1);

    updateDbaseButtons();
//    for (int i=0; i<namesModel->tagIdList.size(); i++) {
//        qDebug() << i << namesModel->tagIdList[i] << namesModel->firstNameList[i] << namesModel->lastNameList[i];
//    }
}
